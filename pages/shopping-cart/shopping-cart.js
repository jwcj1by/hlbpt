...
Page({
  ...
  async onShow() {
    ...
    let res_cart_list = await app.fetch({
      url: api.cart.list,
    })
    let cart_id_list = []
    res_cart_list.list.forEach(element => {
      cart_id_list.push(element.cart_id)
    })

    let options_after = Object.assign(this.data.options, {
      cart_id_list
    })

    page.setData({
      options: options_after
    })
